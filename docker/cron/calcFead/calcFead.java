 ...
 private static void CalcFeadOneYear(Statement stmt, String annee) throws Exception {
    ...
    private static int updateCampagneFeadWithResultSet(Statement stmt,ResultSet rs,String item,String annee) throws Exception {
            ...
            while (rs.next()) {
                switch (item) {
                    case "INIT":
                        query = String.format("insert into campagne_fead(annee,campagne,id_article,id_asso,debut,fin,init,qte,expedie,envoye,cession) values(annee,%s,'%s','%s','%s','%s',%d,%d,%d,%d)",
                                rs.getString("campagne"), rs.getString("id_article"), rs.getString("id_asso"), annee + "-01-01", annee + "-12-31",
                    ...
                    case "CESSIONQTE":
                        query = String.format("insert into campagne_fead(annee,campagne,id_article,id_asso,debut,fin,cession) values(annee,'CUMUL','%s','%s','%s','%s',%s,%s)",
                                rs.getString("id_article"), rs.getString("id_asso"), annee + "-01-01", annee + "-12-31",
                                rs.getString("cession"),rs.getString("qte"));
        ...
        private static void majEnvoye(Statement stmt,String annee) throws Exception {
            ...
            while (rs.next()) {
                ...
                while(rs1.next() && fqte>0) {
                    ...
                    if(dispo>=fqte || getRowCount(rs1)==rs1.getRow()) {
                        rs1.updateString("envoye", String.valueOf(fqte));
                        rs1.updateRow();
                        fqte=0f;
                    }
                    else {
                        rs1.updateString("envoye", rs1.getString("qte"));
                        rs1.updateRow();
                        fqte-=dispo;
                    }
                }
                rs1.beforeFirst();
                int numrows = updateCampagneFeadWithResultSet(stmt,rs,"ENVOYE",annee);
            }
        }
       ...
       private static void majCessions(Statement stmt,String annee) throws Exception {
        ...
        private static void majUneCessionOrigin(Statement stmt, String annee,String origin,String destination,String article,String qte) throws Exception {
            int ucart= 0;
            ...
            if(!qte.equals("0") && !qte.equals("") && (ucart != 0)) {

                int intQte=Integer.parseInt(qte) * ucart;
        ...
        private static void majUneCessionsDestination(Statement stmt,String annee, String campagne ,String article,String asso, String debut,String fin, String qte) throws Exception {
            ...
            if(getRowCount(rs)==0) {
                int tournee = 1;
                addResultSetRow(rs,annee,annee,article,asso,debut,fin,tournee);
            }
        ...
        private static void addCampagneRow(Statement stmt,String annee, String article,String asso,boolean genCumulRow) throws Exception {
            ...
            rs.beforeFirst();
            int numrows = updateCampagneFeadWithResultSet(stmt,rs,"INIT",annee);
            if(genCumulRow) {
                ...
                if(rs2.next()) {
                    ...
                    rs.beforeFirst();
                    numrows = updateCampagneFeadWithResultSet(stmt,rs,"INIT",annee);
                }
            }
        }
